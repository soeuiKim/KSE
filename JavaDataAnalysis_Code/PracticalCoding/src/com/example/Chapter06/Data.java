...
public class Data {
    ...
    public Data(File inputFile) {
        try {
            ...
            x = new double[n];
            y = new double[n];
            minX = minY = Double.POSITIVE_INFINITY; // double중에 가장큰수
            maxX = maxY = Double.NEGATIVE_INFINITY; // 음수중 가장작은수
            for (int i = 0; i < n; i++) {
                ...
                maxY = (yi > maxY? yi: maxY);  //선형방정식 구하는 공식임
            }
            meanX = sX/n;
            meanY = sY/n; //평균값
            slope = (n*sXY - sX*sY)/(n*sXX - sX*sX); //기울기구하는 공식
